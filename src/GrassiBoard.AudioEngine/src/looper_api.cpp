...
{
    if (engine == nullptr || interleaved_stereo_samples == nullptr || frame_count == 0U) {
        return GB_ERROR_INVALID_ARGUMENT;
    }
    try {
        return AsEngine(engine)->LoadLooperMaster(interleaved_stereo_samples, frame_count);
    }
    catch (...) {
...
gb_result GB_CALL gb_looper_set_transport(
    const gb_engine_handle engine,
    const std::uint32_t transport) noexcept
{
    ...
    std::uint32_t* const read_frames) noexcept
{
    if (engine == nullptr || interleaved_stereo_samples == nullptr ||
        capacity_frames == 0U || read_frames == nullptr) {
        return GB_ERROR_INVALID_ARGUMENT;
    }
