...
`v0.6.2` is **Milestone 5 — Virtual Cable PCM Transport** with a Windows 10 capture-activation compatibility fix, while retaining the accepted app/DSP and driver lifecycle:
...
Start live monitoring with a low headset volume. Selecting speakers can create a feedback loop. The v0.6.2 driver is test-signed and distributed separately; read `DRIVER-TESTING.md` inside that package before changing TESTSIGNING.
