...
> **به‌روزرسانی مسیر اجرا — 2026-08-10:** نسخهٔ `v0.9.0` با تأیید صریح کاربر پذیرفته شد. طبق brief ترکیبی، Milestoneهای اصلی `v0.10.0` و `v0.11.0` در یک چرخه ادغام شده‌اند و خروجی عمومی بعدی فقط `v0.11.0` است. این چرخه Profiles، User Voice+Mixer Presets، Global Hotkeys، System Tray، Local Media Deck و اندازه‌گیری/تنظیم latency را پوشش می‌دهد. درایور مجازی سفارشی و capture صدای مرورگر/پردازش خارج از محدوده‌اند؛ مسیر تأییدشدهٔ external VB-CABLE حفظ می‌شود. پس از تولید artifact نسخهٔ `v0.11.0` کار متوقف می‌شود تا تست و تأیید دستی کاربر دریافت شود.
