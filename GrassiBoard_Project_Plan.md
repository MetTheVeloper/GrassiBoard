...
> **انتشار پایدار — 2026-08-10:** پس از تأیید نصب، اجرا، UI و همگام‌سازی نزدیک‌به‌کامل `v1.0.0`، نسخهٔ `v1.0.1` یک کالیبراسیون زنده و ذخیره‌شوندهٔ `-100..+100 ms` برای جبران اختلاف چندمیلی‌ثانیه‌ای وابسته به سیستم اضافه می‌کند و به‌عنوان نخستین release پایدار منتشر می‌شود.

> **وضعیت کاندید نسخهٔ پایدار — 2026-08-10:** پیاده‌سازی `v1.0.0` شامل اصلاح نهایی UI، همگام‌سازی کامل Media با مسیر میکروفون، بازیابی خودکار ورودی در زمان قطع میکروفون، و Installer سفارشی تک‌فایل با کاور رسمی است. مسیر تأییدشدهٔ external VB-CABLE حفظ شده و درایور در پکیج بازتوزیع نمی‌شود؛ Installer فقط در نبود Cable لینک رسمی ناشر را بدون متوقف‌کردن نصب نمایش می‌دهد. این tag تا پایان تست طولانی‌مدت Windows 10 و تأیید نصب/حذف توسط کاربر به‌صورت prerelease باقی می‌ماند.

> **به‌روزرسانی مسیر اجرا — 2026-08-10:** نسخهٔ `v0.9.0` با تأیید صریح کاربر پذیرفته شد. طبق brief ترکیبی، Milestoneهای اصلی `v0.10.0` و `v0.11.0` در یک چرخه ادغام شدند. بازخورد تست `v0.11.0` یک hotfix با نسخهٔ `v0.11.1` تعریف کرد: نام کوتاه endpoint مانیتور، transport iconهای پایدار، Seek حین پخش، و تأخیر شاخهٔ Media به میکروفون مجازی بر اساس latency واقعی الگوریتم Pitch؛ مانیتور هدفون مستقیم باقی می‌ماند. گزارش crash تست v0.11.1 سپس hotfix `v0.11.2` را برای حذف StaticResource تعریف‌نشده، افزودن validation خودکار XAML، و اعمال icon رسمی ارسالی کاربر تعریف کرد. درایور مجازی سفارشی و capture صدای مرورگر/پردازش خارج از محدوده‌اند و مسیر تأییدشدهٔ external VB-CABLE حفظ می‌شود. پس از تولید artifact نسخهٔ `v0.11.2` کار متوقف می‌شود تا تست و تأیید دستی کاربر دریافت شود.
