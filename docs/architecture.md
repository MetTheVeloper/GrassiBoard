...
## v0.9 product boundary

The active product has three layers:

1. `GrassiBoard.App`: WPF `net8.0-windows` x64 application shell, shared state, Sound Pad persistence, offline file decoding, and Mixer controls/presets.
2. `GrassiBoard.AudioEngine`: native C++20 x64 DLL exposing C ABI version 6 and owning WASAPI, Voice DSP, Mixer/Dynamics, the predecoded Soundboard mixer, and meters.
...
Microphone samples are converted to 48 kHz mono float, processed, and written to the existing preallocated mono ring buffer. Render pops processed microphone samples, applies Mic Mute and microphone gain/dynamics, obtains stereo Soundboard samples from `SoundboardMixer`, applies Board gain/ducking, then applies Master gain, linked limiting, clipping protection, and a final safety clamp before rendering to the selected external-cable playback endpoint.
...
`MixerDynamicsProcessor` is preallocated and callback-safe. UI parameters enter through ABI v6 as a fixed-size structure, are published atomically, and are consumed at render-block boundaries. The callback takes no UI lock and performs no allocation. Pitch Wet/Dry reuses the already latency-aligned dry path.
